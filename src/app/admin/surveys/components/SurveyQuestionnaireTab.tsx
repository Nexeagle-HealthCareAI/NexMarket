        ...
        <tbody>
          {questions.map((q) => (
            <tr key={q.id} style={{ borderBottom: '1px solid #e2e8f0', opacity: q.isActive ? 1 : 0.5, transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = '#f8fafc'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
              <td data-label="Order" style={{ padding: '1rem', color: '#64748b', fontWeight: 600, textAlign: 'center' }}>{q.order}</td>
              <td data-label="Key (ID)" style={{ padding: '1rem', color: '#4f46e5', fontWeight: 600 }}>{q.questionId}</td>
              <td data-label="Question Text" style={{ padding: '1rem', color: '#0f172a' }}>
              ...
              <td data-label="Type" style={{ padding: '1rem', color: '#475569', textTransform: 'capitalize', fontWeight: 500 }}>{q.type}</td>
              <td data-label="Required" style={{ padding: '1rem' }}>
