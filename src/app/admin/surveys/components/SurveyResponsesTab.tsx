...
export function SurveyResponsesTab({
  ...
  return (
    <>
      <style>{`
        .survey-table-desktop { display: table; }
        .survey-cards-mobile { display: none; }
        @media (max-width: 768px) {
          .survey-table-desktop { display: none; }
          .survey-cards-mobile { display: flex; flex-direction: column; gap: 1rem; padding: 1rem; }
        }
      `}</style>
      <div style={{ background: 'white', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', overflow: 'hidden', maxWidth: '100%' }}>
        ...
        {/* DESKTOP TABLE */}
        <div style={{ overflowX: 'auto', maxWidth: '100%' }} className="survey-table-desktop">
        ...
        {/* MOBILE CARDS */}
        <div className="survey-cards-mobile" style={{ background: '#f8fafc' }}>
          {paginatedSurveys.map((survey) => {
            let answers: Record<string, unknown> = {};
            try { answers = JSON.parse(survey.answersJson); } catch {}
